...
For the full constructor-form grammar (component registry, bindings, property syntax, etc.) the skill defers to the [constructor-forms](../../../.cursor/rules/constructor-forms.mdc) rule. This skill only encodes the list-form skeleton and the per-data-type column dispatch.
...
2. Replace the `"listFormUid": "<old-uid-or-empty-string>"` line with `"listFormUid": "<new-form.uid>"` using the smallest possible edit targeting the unique `listFormUid` line.
